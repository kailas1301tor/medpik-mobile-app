...
import 'package:medpik/utils/common_widgets/connectivity_observer.dart';
import 'package:medpik/utils/routes/route_constants.dart';
import 'package:medpik/utils/routes/route_generator.dart';

final navigatorKeyProvider =
    Provider((_) => GlobalKey<NavigatorState>());

class MedpikApp extends ConsumerWidget {
  const MedpikApp({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final themeMode = ref.watch(
      themeNotifierProvider.select(
        (asyncValue) => asyncValue.valueOrNull ?? ThemeMode.light,
      ),
    );
